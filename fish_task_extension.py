...
#MongnDB on virtual machine
client = pymongo.MongoClient("mongodb://18.193.70.86:27017/Sparta") # defaults to port 27017
db = client.FishMarketDatabase
...
class Fish:
    ...
    def df_to_dict(self): #Convert dataframe to dictionary
        fish_dict = final_df.to_dict()
        pp(fish_dict)
        return fish_dict

    def upload_to_mongodb(self): #Upload dictionary to MongoDB
        db.FishMarket.drop()
        db.create_collection("FishMarket")
        db.FishMarket.insert_one(fish_dict)
        # print(db.list_collection_names())
        # data = db.FishMarket.find()
        # for d in data:
        #     pp(d)
...
# fish1.upload_to_s3()
fish_dict = fish1.df_to_dict()
fish1.upload_to_mongodb()